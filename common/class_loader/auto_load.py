...
def iter_submodules(path):
    package_name = get_root_package_name(path)
    for name in sorted(iter_submodule_names(path)):
        yield importlib.import_module(package_name + "." + name)


def get_root_package_name(path):
    if __package__:
        marker = ".common.class_loader"
        if __package__.endswith(marker):
            return __package__[:-len(marker)]
